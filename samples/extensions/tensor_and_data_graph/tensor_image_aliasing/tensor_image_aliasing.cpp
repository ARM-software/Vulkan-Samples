...
bool TensorImageAliasing::prepare(const vkb::ApplicationOptions &options)
{
    ...
    prepare_scene_render_target(get_render_context().get_surface_extent().width, get_render_context().get_surface_extent().height);
 ...
 * Also creates a Tensor View (analagous to an Image View).
 */
 ...
 * Also creates a Tensor Views (analagous to an Image View).
 */
